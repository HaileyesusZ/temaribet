    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            if(forgottenPasswords == null){
                response.sendRedirect("invalid_password_reset.jsp");
                return;
            ...
            /*
                Since reset email might expire, it's a must to check against expiration date
            */
            
            Timestamp expirationDate = forgottenPasswords.get(0).getExpirationDate();
            ...
            // Expiration date is not due yet and now it is possible to reset the account's password
            ...
            response.getWriter().println("Error while reseting your account's password, Please try again");
        }
    }
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
            throws ServletException, IOException {
               ...
               if(confirmedPassword.isEmpty() || ! confirmedPassword.equals(password)){
                    request.setAttribute("confirmedPasswordError", ErrorReporter.CONFIRMED_PASSWORD_ERROR);
                    request.setAttribute("urid", resetId);
                    request.setAttribute("urkey", resetKey);
                    request.getRequestDispatcher("password_reset.jsp").forward(request, response);
                    return;
